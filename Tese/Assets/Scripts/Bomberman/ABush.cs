...
public class ABush : GameAgent
{
    //Constructor
    //Receives List<int> (states), int (x), and int (y)
    public ABush(List<int> states, int x, int y)
    {
        ...
        if (states[0] != 0)
        {
            ...
            else
            {
                ...
                foreach (Vector2Int pos in relative_sensors)
                {
                    ...
                    {
                        PutAgentOnGrid(realPos, new ABush(new List<int> { states[0], states[0] }, realPos.x, realPos.y), g);
                        break;
                    }
                }
            }
