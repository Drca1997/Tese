...
public class LifeAgentAlive : Agent
{
    ...
    public override void UpdateAgent(Grid g, int step_stage, System.Random prng)
    {
        switch (step_stage)
        {
            ...
            //this Agent will eliminate itself and create an LifeAgentDead in its place if the rules of Conway's Game of Life so call it
            case 1:
                //eliminates itself and creates an LifeAgentDead in its place
                if (states[0] < 2 || states[0] > 3)
                {
                    EliminateAgent(this, g, step_stage, prng);
                    PutAgentOnGrid(position, new LifeAgentDead(new List<int> {0}, position.x, position.y), g);
                }
                break;
        }
