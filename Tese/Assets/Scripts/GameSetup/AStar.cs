...
public static class AStar
{
    ...
    public static List<Action> AStarForPlanning(PlanningAgent agent, GraphNode start, GraphNode end, Func<ActionStateGraphNode, bool> IsObjective, Action[] possibleActions, Func<ActionStateGraphNode, ActionStateGraphNode, double> heuristic)
    {
        List<GraphNode> expandableList = new List<GraphNode> { start }; //pega-se no primeiro nó
        List<GraphNode> visitedList = new List<GraphNode>();
      
        start.HCost = heuristic((ActionStateGraphNode)start, (ActionStateGraphNode)end);
        start.FCost = start.GCost + start.HCost;
        int ind = 2;
        ...
        while (expandableList.Count != 0) //enquanto houver nós para serem visitados
        {
            /*
            Debug.Log("EXPANDABLE LIST:");
            DebugAStarList((GraphNode)expandableList);
            Debug.Log("VISITED LIST:");
            DebugAStarList(visitedList);
            */
            /*
            logger.WriteLine("EXPANDABLE LIST  with " + expandableList.Count + " nodes");
            foreach (ActionStateGraphNode no in expandableList)
            {
                if (no.ActionTakenToReachHere != null)
                {
                    logger.WriteLine(Utils.ActionToString(no.ActionTakenToReachHere.RawAction));
                    logger.WriteLine("To achieve this grid:");
                }
                logger.WriteLine("Nó " + no.Index);
                logger.WriteLine(no.DebugWorld());
            }
            DebugAStarList(expandableList);
           
            logger.WriteLine("VISITED LIST with " + visitedList.Count + " nodes");
            foreach(ActionStateGraphNode no in visitedList)
            {
                if (no.ActionTakenToReachHere != null)
                {
                    logger.WriteLine(Utils.ActionToString(no.ActionTakenToReachHere.RawAction));
            ...
            */
            ...
            if (IsObjective(node))  //verifica-se se é objetivo
            {
                Debug.Log("OBJETIVO ENCONTRADO");
            ...
            //
            //List<GraphEdge> edges = graph.EdgesAdjacencyListVector[node.Index];
            List<Action> actions = GetPossibleActions(possibleActions);
            ...
            for (int i = 0; i < actions.Count; i++)
            {
                ...
                if (childNode.HCost == int.MaxValue) //calcula hCost se ainda não o foi
                {
                    childNode.HCost = heuristic(childNode, (ActionStateGraphNode)end);
                    //Debug.Log("H DE NÓ " + childNode.Index + "= " + childNode.HCost);
                }
                double fCost = childNode.HCost + gCost;
    ...
    private static List<Action> GetPlan(ActionStateGraphNode node)
    {
        ...
        actionsTaken.Add(node.ActionTakenToReachHere);
    ...
    private static List<Action> GetPossibleActions(Action[] possibleActions)
    {
